...
local function yank_helper(register, linewise, type, pattern)
  ...
  cmd = cmd .. '"' .. register .. type

  if linewise then
    cmd = 'V' .. cmd
  else
    cmd = '' .. vim.o.textwidth .. 'l' .. cmd
  end
...
function M.comment_at_end()
  vim.api.nvim_set_current_line('x' .. vim.api.nvim_get_current_line())
  vim.cmd.normal('gcc')

  local line = vim.api.nvim_get_current_line()
  local comment = line:match('[^ ]*')
  local pattern = comment:gsub('%W', '%%%1') -- magic scape

  pattern = '^%s*' .. pattern .. '%s*(.-)%s*$'
  line = line:gsub(pattern, '%1'):sub(2)
